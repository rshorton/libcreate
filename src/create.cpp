...
float vel_yaw_sum;
  ...
  void Create::onData() {
    ...
    if (delta_times.size() > NUM_DELTA_TIMES) {
    	delta_time_sum -= delta_times.back();
    ...
    if (vel_x_hist.size() > NUM_VEL_HIST_SAMPLES) {
    	vel_x_sum -= vel_x_hist.back();
    	vel_x_hist.pop_back();
    }
    vel.x = vel_x_sum/(float)vel_x_hist.size();

    vel_yaw_hist.push_front(vel.yaw);
    vel_yaw_sum += vel.yaw;
    if (vel_yaw_hist.size() > NUM_VEL_HIST_SAMPLES) {
    	vel_yaw_sum -= vel_yaw_hist.back();
    	vel_yaw_hist.pop_back();
    }
